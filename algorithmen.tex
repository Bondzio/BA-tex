...
%\eeq
\beq
F=\sum_{i=1}^n\left(y_i-\left(\hat y_i^{\left(t-1\right)}+f_t\left(x_i\right)\right)\right)^2+\sum_{i=1}^t\Omega\left(f_i\right)
\label{eq:xgb_zielfkt}
\eeq	
Dabei ist
\beq
\Omega\left(f_t\right)=\gamma T+\frac12\lambda\sum_{j=1}^T w_j^2
\label{eq:complexity}
\eeq
